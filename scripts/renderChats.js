...
function renderChats(arr) {
  ...
  const renderChats = (chats) => {
    return (
      chats.map((chat)=> {
        return (
          <SimpleCell key={chat.id} style={{borderRadius: 15}} borderRadiusMode={'auto'} className="bg-grey bg-opacity-5 flex my-3 rounded-2xl"  onClick={() => router.push('/chat')} before={<Image alt='' src={ChatAvatar} height={70}/>}>
